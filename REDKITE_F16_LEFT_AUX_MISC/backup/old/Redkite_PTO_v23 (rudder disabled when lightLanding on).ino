...
// #define BAUDRATE    115200 /* faster speed in bps */
...
int RudderPedal(int a1, int a2, int lightLanding)
{
  // analog inputs, 0 to 1023
  static int lb_min = 300, lb_max = 750, rb_min = 300, rb_max = 750;
  ...
  if ( lightLanding == -1 || lb > DEADZONE && rb > DEADZONE ) {          // if landingLight is up position (on runway) or 2 pedal pressed simultaneously more than deadzone, act as left and right brake 
#if JOYSTICK_SIZE == 12
...
#elif JOYSTICK_SIZE == 64
    Joystick.Xrotate(lb);                                            // left brake
    Joystick.Yrotate(rb);                                           // right brake
    // Joystick.Zrotate( (1023 - lb + rb) / 2 );                           // opt1) calculated rudder position while braking
    Joystick.Zrotate( 1024 / 2 );                                    // opt2) rudder at center position while braking
#endif
    ...
    // Joystick.Zrotate( 1024 / 2 );                                    // opt2) rudder at center position while braking
#elif JOYSTICK_SIZE == 64
    Joystick.Xrotate(0);                                            // left brake
    Joystick.Yrotate(0);                                           // right brake
    Joystick.Zrotate( (1023 - lb + rb) / 2 );                           // opt1) calculated rudder position while braking
    // Joystick.Zrotate( 1024 / 2 );                                    // opt2) rudder at center position while braking
#endif
  }
    ...
    if (readch > 0) {
        switch (readch) {
            ...
            case '\r': // Return on CR
                ...
                return rpos;	// return strlen
            default:	// Something received
                if (pos < len-1) {
                    buffer[pos++] = readch;
                    buffer[pos] = 0;
                }
        }
		return 0;	// Something received
    }
    // nothing received
    return -1;	
}

int ProcessF4TS()                                              // this code is from F4TS arduino code
{
  int ret;

  static char buffer[BUFFERSIZE];
  if ( (ret = readline(Serial.read(), buffer, BUFFERSIZE)) > 0) {
...
void lightBitSet(int nose, int left, int right, int ecm) 
{
  // Gear toggle switch changed, but F4TS setLightbit not received within 6 second timeout
  // So, simulate Gear LED

  digitalWrite(8, nose); digitalWrite(9, left); digitalWrite(10, right); 
  digitalWrite(11, ecm);      
  
  // Serial.printf("F4TS offline: ecm = %d\n", ecm);
...
void loop() {
  ...
  lightLanding = Toggle3P(&btn, 4, 5);                                                   // Light Landing, Off, Taxi
  ...
  RudderPedal(A10, A11, lightLanding);                                                  

  // Analog knobs
  int a1, a2, a3;

#if JOYSTICK_SIZE == 12
  Joystick.X((a1 = analogRead(A15)));                                            // HDG knob
  Joystick.Y((a2 = analogRead(A16)));                                            // CRS knob
  Joystick.Z((a3 = analogRead(A17)));                                            // ALT knob
#elif JOYSTICK_SIZE == 64
  Joystick.X((a1 = analogRead(A15))*64);                                            // HDG knob
  Joystick.Y((a2 = analogRead(A16))*64);                                            // CRS knob
  Joystick.Z((a3 = analogRead(A17))*64);                                            // ALT knob
#endif
  // Serial.printf("%d, %d, %d\n", a1, a2, a3);
  ...
  while ( ProcessF4TS() >= 0 ) {                                     // something was received from F4TS
	// Something received. Reset heartbeat to 0 
	// DED message is received for every 1 second. So, it is considered as heartbeat. DED should be set in F4TS config for correct F4TS detection.
    heartbeat = 0;                                                         
    // Serial.printf("receiving msg from Falcon BMS: ret = %d\n", ret);
  } 
  
  if ( heartbeat >= (1000/DELAY) * F4TSTIMEOUT ) { // F4TS timeout
  	static int blink;

    // set Gear LED, EMC led blink
    lightBitSet(gearSW, gearSW, gearSW, blink++/(1000/DELAY/2));
    blink = blink % (1000/DELAY);
    // Serial.printf("blink = %d, blink / (1000/DELAY) = %d\n", blink, blink/(1000/DELAY));
  }
