...
int RudderPedal(int a1, int a2)
{
  ...
  lb = max(min(lb, lb_max), lb_min);                                    // trim initial outstanding value beyond min, max
  rb = max(min(rb, rb_max), rb_min);
...
void loop() {
  ...
  static int heartbeat, ecm;
  int ret;

  while ( (ret = ProcessF4TS()) >= 0 ) {                                     // something was received from F4TS
