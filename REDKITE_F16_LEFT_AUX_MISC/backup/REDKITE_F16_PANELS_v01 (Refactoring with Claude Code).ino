...
#define TOGGLE_HOLD   true      // false = momentary pulse, true = hold

// ================================================================
//  Type Definitions
// ================================================================

enum SwitchType { SW_PUSH, SW_ON_ON, SW_ON_OFF_ON };
...
struct SwitchDef {
  ...
  // Button count (HOLD mode): SW_PUSH=1, SW_ON_ON=1, SW_ON_OFF_ON=2
  // Button count (MOMENTARY): SW_PUSH=1, SW_ON_ON=2, SW_ON_OFF_ON=3
};
...
const SwitchDef switches[] = {
  ...
  // ---- Standalone ----
  {"Eject",                 PNL_NONE,       SW_PUSH,       0,   0,  NULL},

  // ---- Gear Panel (left aux console) ----
  {"EMER Jettison",         PNL_GEAR,       SW_PUSH,       1,   0,  NULL},
  {"Store CAT",             PNL_GEAR,       SW_ON_ON,      2,   0,  NULL},   // CAT I / CAT III
  {"Horn Silencer",         PNL_GEAR,       SW_PUSH,       3,   0,  NULL},
  {"Landing Light",         PNL_GEAR,       SW_ON_OFF_ON,  4,   5,  &swLandingLight},
  {"Hook",                  PNL_GEAR,       SW_ON_ON,      6,   0,  NULL},   // Up / Down
  {"Landing Gear",          PNL_GEAR,       SW_ON_ON,      7,   0,  &swGear},
  // {"DN LOCK REL",           PNL_GEAR,       SW_PUSH,       0,   0,  NULL},   // TODO: assign pin
  // {"GND JETT ENABLE",       PNL_GEAR,       SW_ON_ON,      0,   0,  NULL},   // ENABLE / OFF — TODO: assign pin
  // {"ANTI SKID",             PNL_GEAR,       SW_ON_OFF_ON,  0,   0,  NULL},   // PARKING BRAKE / ANTI SKID / OFF — TODO: assign pin
  // {"BRAKES Channel",        PNL_GEAR,       SW_ON_ON,      0,   0,  NULL},   // CHAN 1 / CHAN 2 — TODO: assign pin

  // ---- Alt Gear Panel (left aux console) ----
  // {"ALT GEAR Handle",       PNL_ALT_GEAR,   SW_PUSH,       0,   0,  NULL},   // TODO: assign pin
  // {"ALT GEAR Reset",        PNL_ALT_GEAR,   SW_PUSH,       0,   0,  NULL},   // TODO: assign pin

  // ---- MISC Panel (center console, left of left MFD) — commented out ----
  {"Laser ARM",             PNL_MISC,       SW_ON_ON,     28,   0,  NULL},
  ...
  // {"ALT REL",               PNL_MISC,       SW_PUSH,       0,   0,  NULL},
  // {"ADV MODE",              PNL_MISC,       SW_PUSH,       0,   0,  NULL},
  ...
  // {"CMDS RWR",              PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS JMR",              PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS MWS",              PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS O1",               PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS O2",               PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS CH",               PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS FL",               PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  // {"CMDS JETT",             PNL_CMDS,       SW_ON_ON,      0,   0,  NULL},
  ...
  // {"ECM RESET",             PNL_ECM,        SW_PUSH,       0,   0,  NULL},
  // {"ECM BIT",               PNL_ECM,        SW_PUSH,       0,   0,  NULL},
  ...
  // {"ELEC CAUTION RESET",    PNL_ELEC,       SW_PUSH,       0,   0,  NULL},
};
...
const char* const ecmBtnNames[] = {"ECM 1","ECM 2","ECM 3","ECM 4","ECM 5","ECM BLANK","ECM FRM","ECM SPL"};
...
static int prevPinState[MAX_PIN];
...
int switchButtonCount(SwitchType type) {
  if (TOGGLE_HOLD) {
    switch (type) {
      case SW_PUSH:       return 1;
      case SW_ON_ON:      return 1;
      case SW_ON_OFF_ON:  return 2;
    }
  } else {
    switch (type) {
      case SW_PUSH:       return 1;
      case SW_ON_ON:      return 2;
      case SW_ON_OFF_ON:  return 3;
    }
  }
  return 1;
}
...
void processSwitches() {
  for (unsigned int i = 0; i < NUM_SWITCHES; i++) {
    ...
    switch (sw.type) {
      case SW_PUSH: {
        int pressed = !digitalRead(sw.pin1);
        Joystick.button(btn, pressed);
        if (sw.stateRef) *sw.stateRef = pressed;
        break;
      }
      ...
      case SW_ON_ON: {
        ...
        if (TOGGLE_HOLD) {
          ...
          Joystick.button(btn,     prevPinState[sw.pin1] == HIGH && state == LOW);
          Joystick.button(btn + 1, prevPinState[sw.pin1] == LOW  && state == HIGH);
          prevPinState[sw.pin1] = state;
        }
      ...
      case SW_ON_OFF_ON: {
        ...
        if (TOGGLE_HOLD) {
          ...
          int toUp     = prevPinState[sw.pin1] == HIGH && s1 == LOW;
          int toDown   = prevPinState[sw.pin2] == HIGH && s2 == LOW;
          int toCenter = (prevPinState[sw.pin1] == LOW && s1 == HIGH) ||
                         (prevPinState[sw.pin2] == LOW && s2 == HIGH);
          prevPinState[sw.pin1] = s1;
          prevPinState[sw.pin2] = s2;
          Joystick.button(btn,     toUp);
          Joystick.button(btn + 1, toCenter);
          Joystick.button(btn + 2, toDown);
        }
...
int processF4TS() {
  ...
  int ret = readSerialLine(Serial.read(), buffer, BUFFERSIZE);
...
void setup() {
  ...
  Serial.println("=== Redkite_LeftPanel ===");
...
void loop() {
  ...
  // --- F4TS communication & LED control ---
