...
- **Upload**: Open `REDKITE_F16_PANELS.ino` in Arduino IDE and Upload
...
All hardware is declared in config arrays in `REDKITE_F16_PANELS.ino` under the **HARDWARE CONFIGURATION** section (~line 99-244). To add/remove hardware, only edit these arrays:
...
| File | Purpose |
|------|---------|
| `REDKITE_F16_PANELS.ino` | Main sketch: config arrays, MCP23017 driver, switch/analog/pedal processing, protocol detection, main loop |
